...
## 目的
- 本ドキュメントでは、顔ランドマークから皮膚（両頬）の関心領域（ROI）を特定して RGB 信号を置き換え抽出するとともに、POS（Plane-Orthogonal-to-Skin）アルゴリズムを用いて皮膚の色調変化から脈波信号を推定する `MyAnalysisrPPG` クラスの動作原理と利用方法について解説します[cite: 8]。

## 前提条件
- **【重要】** `my_analysis_rppg.py` がライブラリフォルダー（例: `my_libs/`）に配置されていることを確認してください[cite: 8]。
- **【重要】** 内部処理に `numpy` を使用します[cite: 8]。
...
* 顔ランドマークの「小鼻」と「目尻」の中間点を中心として、輪郭や鼻の影を回避する絶対安全圏に左右の頬 ROI を配置します。
...
```python
import numpy as np
from my_libs.my_analysis_rppg import MyAnalysisrPPG

def main():
    # 30 fps 設定でインスタンス化
    rppg_analyzer = MyAnalysisrPPG(fps=30.0)[cite: 8]

    # ダミーのカメラフレーム (480x640, BGR 3チャンネル)
    dummy_frame = np.full((480, 640, 3), 128, dtype=np.uint8)

    # 68点顔ランドマークのダミーデータ（配列インデックスに対応する [x, y]）
    dummy_landmarks = {i: (300 + (i % 5) * 10, 200 + (i // 5) * 10) for i in range(68)}[cite: 8]
    # 主要な参照点をダミー調整（目・小鼻付近）
    ...
    cheeks = rppg_analyzer.extract_cheek_rois(dummy_frame, dummy_landmarks)[cite: 8]
    print(f"右頬 RGB: {cheeks['right_rgb']}, 矩形: {cheeks['right_rect']}")[cite: 8]
    print(f"左頬 RGB: {cheeks['left_rgb']}, 矩形: {cheeks['left_rect']}")[cite: 8]

    # 2. 時系列 RGB データからの POS 脈波信号の抽出（例: 60フレーム分のデータ）
    # 実際のアプリではフレーム毎に平均RGBを蓄積した時系列アレイを使用します
    dummy_rgb_history = np.random.normal(loc=120.0, scale=2.0, size=(60, 3))[cite: 8]

    # 脈波時系列全体を計算
    pos_signal = rppg_analyzer.compute_pos_signal(dummy_rgb_history)[cite: 8]
    # 最新の生値 1 点を取得
    latest_pos = rppg_analyzer.get_latest_pos_value(dummy_rgb_history)[cite: 8]

    print(f"算出された脈波データ長: {len(pos_signal)}")[cite: 8]
    print(f"最新の脈波値: {latest_pos:.4f}")[cite: 8]
...
| `extract_roi_by_rect(frame, x, y, w, h)` | `frame`: 画像フレーム<br><br>`x, y, w, h`: 切り出し領域 | `tuple` | 指定範囲を画角内に安全クリップし、平均 RGB 値（`rgb_mean`）と補正後の有効矩形（`actual_rect`）を返却。 |
| `extract_cheek_rois(frame, landmarks)` | `frame`: 画像フレーム<br><br>`landmarks`: 68点顔座標 | `dict` | 両目・小鼻の幾何位置からノイズの少ない左右頬の ROI 座標および平均 RGB 値をまとめて返却。 |
